...
const Card = ({task}) => {
    ...
    return ( <div className={`rounded-md bg-gray-800 shadow-md p-2 flex flex-col justify-between m-2 ${colors[task.status]}`} >
        ...
        <div className="flex my-1">
            {task.participants.map((part,index)=>(
                <div key={index}  className={`w-6 h-6 rounded-full mr-1 flex justify-center items-center text-sm font-bold ${index%3===0?`bg-green-500`:index%3===1?`bg-blue-500`:`bg-orange-500`}`}>{part[0].toUpperCase()}</div>
            ))}
        </div>
    </div> );   
}
